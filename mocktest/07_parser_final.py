...
# ---------------------------------------------------------
# 1. 파일 이름 정확히 입력
# parser = AdvancedManualParser("app/docs/LX3HEV_2026_ko_KR.pdf")
parser = AdvancedManualParser("app/docs/_2025_ko_KR.pdf")
...
# 내용이 있고, 표도 있는 알짜배기 Chunk를 찾아서 보여줌
sample_count = 0
for chunk in chunks:
    if len(chunk['tables']) > 0 and sample_count < 3:
        print(f"\n📄 Page {chunk['page']} | 📌 제목: {chunk['heading']}")
        # print(f"📝 본문 일부: {' '.join(chunk['content'][:100])}...")
        # print(f"📊 포함된 표(Markdown):\n{chunk['tables'][0][:400]}...") # 표 앞부분만
        # 전체 본문과 표 출력
        print(f"📝 본문 일부: {' '.join(chunk['content'])}...")
...
if sample_count == 0:
    ...
    for chunk in chunks[:3]:
        print(f"\n📄 Page {chunk['page']} | 📌 제목: {chunk['heading']}")
        print(f"📝 본문 일부: {' '.join(chunk['content'][:300])}...")
